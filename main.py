...
# ✅ CORS Middleware (aplicado inmediatamente después de app)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # O usa ["http://18.210.82.24"]
...
def crear_socio(socio: SocioCreate, db: Session = Depends(get_db)):
    existente = db.query(models.Socio).filter(models.Socio.email == socio.email).first()
    if existente:
...
@app.post("/reservas/", response_model=schemas.ReservaCreate)
...
@app.post("/favoritos/", response_model=schemas.FavoritoCreate)
def agregar_a_favoritos(favorito: schemas.FavoritoCreate, db: Session = Depends(get_db)):
